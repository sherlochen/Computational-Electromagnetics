function  RCS1 =  PO_function(f,filename)
%% ***********************
% 设置入射波
c0 = 3e8;%光速
% f = 1e9;%入射波频率
theta  = 0;%入射角度设置theta phi
phi = 0;
lamda = c0/f;
K0 = 2*pi/(c0/f);%波数
...
RCS1 = 10*log(4*pi*R^2*(abs(ES1(1,1))+abs(ES1(1,2))+abs(ES1(1,3)))^2);
%% ***************************************
ES2 = -1i*K0*exp(-1i*K0*R)/(2*pi*R)*sum(triangle_Rcs).*Ki_E;
RCS2 = 10*log(4*pi/(lamda^2)*(abs(sum(triangle_Rcs)))^2);
end
