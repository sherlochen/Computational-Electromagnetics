...
K0 = 2*pi/(c0/f);%波数
phi = 10;
theta  =0:5;%入射角度设置theta phi
for j = 1:length(theta)
    Ki_x(j) = sin(theta(j)/180*pi)*cos(phi/180*pi);
    Ki_y(j) = sin(theta(j)/180*pi)*sin(phi/180*pi);
    Ki_z(j) = cos(theta(j)/180*pi);
    %Ki_n(j) = [Ki_x(j),Ki_y(j),Ki_z(j)]/norm([Ki_x(j),Ki_y(j),Ki_z(j)]);%入射方向的单位矢量
    %Ks_n(j) = -Ki_n(j);%散射方向矢量
    % ********************************
    %Ki_EH = null(Ki_n);
    %Ki_E = Ki_EH(:,1)'; %入射电场单位矢量，在PO算法中不需要考虑计划特性
    %% ***********************
% 导入模型网格
% v包含所有三角形的顶点[3 * n x 3] 
...
% filename = "C:\Users\LXW\Desktop\物理光学法\算例\sphere\sphereUG_wangge.stl";
% filename = "C:\Users\LXW\Desktop\物理光学法\算例\complex_model\complex_model.stl";
filename = "F:\物理光学法\算例\sphere\wangge.stl";
% filename = "C:\Users\001\Desktop\群目标计算程序一体化\cylinder\cylinder1.stl";
...
h = waitbar(0,'PO计算中...');
    ...
    triangle_R(i) =  dot(Ki_n,triangle_Z_points(i,:));   % 计算各面元中心到坐标原点的距离在入射方向上的投影；r'*ki
    ...
    % ********************************************************************
    triangle_E(i,:) = exp(-1i*K0*triangle_R(i)).*Ki_E; %面元入射电场
    triangle_H(i,:) = cross(Ki_n,triangle_E(i,:));     %面元入射磁场
    if(triangle_O1(i)==1)    
          triangle_J(i,:) = cross(n(i,:),triangle_H(i,:));%面元表面电流
          triangle_Es(i,:) = cross(triangle_J(i,:),Ks_n)*exp(1i*K0*triangle_Rs(i))*triangle_S(i); %面元远场
...
RCS1 =4*pi*R^2*(abs(ES1(1,1))+abs(ES1(1,2))+abs(ES1(1,3)))^2;
%% ***************************************
